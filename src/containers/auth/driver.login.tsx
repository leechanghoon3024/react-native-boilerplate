...
const DriverLogin = () => {
    ...
    const joinHandler = async (value: { email: string; password: string }) => {
        ...
        try {
            const api = await axiosService.post('/users/login/driver', { username: email, password });
            const { data } = api.data;
                        ...
                        return (
                            ...
                            >
                                <Text color={'white.100'} textAlign={'center'}>
                                    Can't log in. Please wait for approval
                                </Text>
                            </Box>
                        );
                    },
    ...
    return (
        <>
            <Box flex={1}>
                <DriverLoginHeader navigation={navigation} backOption={true} />
                        ...
                        <Box flex={1}>
                            <Stack space={2} width={'100%'}>
                                ...
                                <Button
                                    ...
                                    onPress={() => navigation.navigate('SingUpScreen' as never)}
                                    variant={'basicButton'}
                                >
                                    <HStack alignItems={'center'}>
                                        <Box alignItems={'center'} width={'100%'}>
