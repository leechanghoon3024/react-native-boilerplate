...
import { La31B } from '../../themes/font.style';
...
export default class OTPInputView extends Component {
    constructor(props) {
        ...
        this.renderOneInputField = (_, index) => {
            ...
            const digitIndex = digits.length >= index;
            return (
                <Box
                    pointerEvents="none"
                    key={index + 'view'}
                ...
                >
                    <Input
                        {...La31B}
                        testID="textInput"
                        borderBottomColor={digitIndex ? '#16A6DA' : 'gray.200'}
                        borderWidth={0}
                        borderBottomWidth={2}
                        borderRadius={0}
                        ref={(ref) => {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.code !== code) {
            this.setState({ digits: nextProps.code });
        }
    }
    componentDidMount() {
        this.copyCodeFromClipBoardOnAndroid();
