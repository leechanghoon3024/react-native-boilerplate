...
const GoogleAdd = () => {
    ...
    const [addressName, setAddressName] = useState({ address: '', addressMore: '' });
    ...
    const insertApi = async (value: any) => {
        ...
        const { addressMore, address } = addressName;

        try {
            const api = await axiosService.post('/users/app/address/insert', { lot, lat, type, moreText: typeText, address, addressMore });
    ...
    const getAddressInfo = async () => {
        ...
        if (status) {
            const { address, addressMore, moreText, lat, lot, type } = data;
            setSelectAddress(true);
            setAddressName({ address, addressMore });
                ...
                {step === 1 ? (
                    <>
                        <Box my={4} flex={1} pt={'30px'} px={'20px'}>
                            <Heading mx={'10px'} mb={'66.3px'} {...Ar36B} fontSize={'30px'} color={'black.100'}>
                                {'Pickup address'}
                            </Heading>
                            {selectAddress ? (
